...
async function getHomeCategory(req) {
    ...
    if (req.body.userId) {
        ...
        project['$project']['isFavorite'] = {
            // $in: [ mongoose.Types.ObjectId(req.body.userId), "$userFavData.favourites" ]
            $cond: {
                if: {
...
async function homePage(req) {
    ...
    if (req.body.userId) {
        ...
        newlyAddedQuery.push(lookup)
    } else if(!req.body.userId){
        project['$project']['isFavorite'] = "false"
    }
    const dastinationQuary = []
    let favDatination = {
        $lookup: {
            from: 'accomodations',
    ...
    dastinationQuary.push(favDatination)
    const favDastination = {
        $group: {_id: '$amdData', count: {$sum:1}}
    }
    dastinationQuary.push(favDastination)
    const limits = {
        $limit: 4
    }
    dastinationQuary.push(limits)
    const count = {
        $sort : {
            count: -1
        }
    }
    dastinationQuary.push(count)

    const unwind = {
        $unwind: '$_id',
   }

   dastinationQuary.push(unwind)
    let projectDastination = {
        $project: {
            type: "$_id.type",
            _id: 0,
            media: "$_id.media",
    ...
    dastinationQuary.push(projectDastination)

    const topDastinationQuery = [
        {
            $lookup: {
                from: 'bookings',
                localField: "propertyId",
    ...
    let HotFavDestination = await amdDao.getDastination(dastinationQuary)
    let dastination = await amdDao.getTopRated(topDastinationQuery)
    let response = {
        ...
        hotFavDestination:HotFavDestination,
        dastination: dastination
    }
...
async function searchnFilterHomeOffice(req) {
    ...
    let project = {
        $project: {
            ...
            type: "$type",
            name: "$name",
            profilePicture: "$landlordData.profilePicture",
            totalRatings: { $ifNull: [{ $avg: "$reviewData.ratings" }, 0] },
            totalRatingsBy: { $size: "$reviewData" },
            totalQuantity: '$quantity',
            totalReview: "$reviewData",
    ...
    if(req.body.aminities) {
        searchQuery.push({ "$match" : { "amenities.amt": { "$in": req.body.aminities } } })
     }
...
async function searchnFilterWorkPlace(req) {
    ...
    let project = {
        $project: {
            ...
            totalRatingsBy: { $size: "$reviewData" },
            totalQuantity: '$quantity',
            amenities: { $eq: [{$arrayElemAt: ["$amenities.amt", 1]}, "WiFi"] },
            country: "$address.country",
            city: "$address.city",
            location: "$address.location",
            state: "$address.state",
    ...
    return amdDao.getAccomodation(searchQuery).then((response) => {
        ...
        if (response && response.length) {
            response.map((property) => {
                ...
                if (req.body.guests) {
                    console.log('guestSum guestSum', guestSum)
                    if (!guestSum && guestSum >= req.body.guests) {
                        properties.push(property)
                    } else {
                        const index = response.indexOf(property);
                        if (index > -1) {
            ...
            obj.response = response
            obj.totalRecord = totalRecord.length
            obj.limit = option.limit
...
async function getNearByMostSearch (req) {
    const filter = req.body.filter
    const limit = {
        $limit: 10
    }
    const getNearByQuery = []
    if(filter && filter.latitude && filter.longitude) {
        getNearByQuery.push({
            $geoNear: {
                near: {
                    type: "Point", 
    ...
    getNearByQuery.push(match)
    getNearByQuery.push(lookup[0])
    let project = {
        $project: {
            type: "$type",
            name: "$name",
    ...
    getNearByQuery.push(project)
    getNearByQuery.push(limit)
    const recentSerchQuery = [{
        $sort: {
            createdAt: -1
        }
    }, {
    ...
    popularCitiesQuery.push(popularCity)
    const popularCities = {
        $group: {_id: '$amdData', count: {$sum:1}}
    }
    popularCitiesQuery.push(popularCities)
    ...
    popularCitiesQuery.push(count)

    const unwind = {
        $unwind: '$_id',
   }

   popularCitiesQuery.push(unwind)
    let projects = {
        $project: {
            type: "$_id.type",
            _id: 0,
    ...
    popularCitiesQuery.push(projects)
    popularCitiesQuery.push(limit)
    const searchData = await amdDao.getresentlySerch(recentSerchQuery)
    const nearByLocation = amdDao.getTopRated(getNearByQuery)
    const popularCitiesData = await amdDao.getDastination(popularCitiesQuery)

    const nearLocation = []
    if(nearByLocation._rejectionHandler0 && nearByLocation._rejectionHandler0.length > 0) {
        nearByLocation._rejectionHandler0.map((e, i) => {
           const cor = nearByLocation._rejectionHandler0[i].location.coordinates
           const distance = appUtils.getDistanceFromLatLonInKm(filter.latitude, filter.longitude, cor[0], cor[1])
           nearLocation.push({...e, distanceKm : Math.round(distance)})
        })
    }

    nearLocation.sort(function(a, b) {
    ...
    const response = {
        ...
        recentlySearch: searchData
    }
    return amdMapper.responseMappingWithData(amdConstants.CODE.ok, amdConstants.MESSAGE.success, response);
}

async function getDastination (req) {
    const dastinationQuary = []
    let favDatination = {
        $lookup: {
            from: 'accomodations',
   ...
   dastinationQuary.push(unwind)
    ...
    const mostFavDestination = await amdDao.getDastination(dastinationQuary)
    const dastination = await amdDao.getTopRated(topDastinationQuery)

    const obj = {
        HotFavDestination: mostFavDestination,
        dastination: dastination
    }

...
async function getNearByMostSearch (req) {
    ...
    const searchData = await amdDao.getresentlySerch(serchQuery)
    const nearByLocation = amdDao.getTopRated(getNearByQuery)
    const response = {
        resentlySearch : searchData,
...
async function getDastination (req) {
    ...
    const topDastination = {
        $group: {_id: '$amdData.address.city', count: {$sum:1}}
    }
    dastinationQuary.push(favDastination)
   ...
   dastinationQuary.push(unwind)
    ...
    const obj = {
        mostFavDestination: mostFavDestination,
        dastination: dastination
    }
